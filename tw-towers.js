...
(function(){
  ...
  highline:{
    ...
    s2:'Picture towers rising across a fast-growing market, many starting with one anchor tenant, then adding a second and third as networks densify. The <b style="color:#0c6b4f">return</b> rises with the tenancy ratio, each added tenant near-pure margin. Drag the towers, the rent per tenancy and the tenancy ratio, the lease-up runway is large, but the whole return is in reais, discounted like an emerging-market asset.',
    driverLab:'Rent / tenancy', availLab:'Tenancy ratio', hrK:'Lease revenue', yrS:'tenancies × rent',
    ledge:{a:'+ anchor rent',b:'+ colocation',c:'− site opex'}, demandLabel:'TENANCY',
    preset:'Load Highline',
    try:'<b>Try this:</b> raise the <b>tenancy ratio</b>, in an emerging market many towers start single-tenant, so the lease-up upside (adding a second or third carrier at near-100% margin) is the entire growth story. But the whole return is in reais: push the cost of debt and watch a strong nominal number net down once discounted like a Brazilian asset. That tension is the EM towerco trade.',
    ...
    s6:'Highline is colocation upside at an EM discount rate. What drives it:',
    breakers:['<b>Lease-up</b>, adding colocation tenants to single-tenant towers is the near-100%-margin upside.','<b>Build-to-suit</b>, coverage and 5G density extend the tower base.','<b>Country &amp; currency</b>, Brazilian rates and the real, not the towers, set the discount rate.','<b>Carrier health</b>, three large tenants underpin demand; their capex plans matter.'],
  ...
  amplitel:{
    ...
    s4a:'Per tower the cost is fixed, ground lease, power, maintenance, so the anchor master lease already covers it and the margin is high. The colocation upside is almost pure EBITDA: a second carrier on a Telstra tower adds rent against a cost the anchor pays. The defining feature today is the secure anchor base; the growth lever is the tenancy ratio.',
    ...
    s6:'Amplitel is the anchored carve-out with lease-up to come. What drives it:',
  ...
  tasc:{
    ...
    s2:'Picture a region early in tower sharing, masts going up for coverage, many still single-tenant. The <b style="color:#0c6b4f">return</b> today is anchored by build-to-suit rent; the prize is the <b>tenancy ratio</b> climbing as carriers colocate. Drag the towers, the rent per tenancy and the tenancy ratio, a low base with a long lease-up runway. Figures are illustrative.',
    driverLab:'Rent / tenancy', availLab:'Tenancy ratio', hrK:'Lease revenue', yrS:'tenancies × rent',
    ledge:{a:'+ anchor rent',b:'+ colocation',c:'− site opex'}, demandLabel:'TENANCY',
    preset:'Load TASC',
    ...
    s4a:'Per tower the cost is fixed, ground lease, power, maintenance, so even at a lower tenancy the first tenant covers the site and the margin is healthy. The upside is the lease-up runway: as the ratio climbs from a low base, each added carrier is almost pure EBITDA. The defining feature is the build-out pipeline today and the tenancy ratio climbing tomorrow.',
    ...
    s6:'TASC is the colocation flywheel at the start. What drives it:',
